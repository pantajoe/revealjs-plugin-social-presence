...
/** @param {Node} node */
function getPathSegment(node: Node) {
 ...
 * @param {number} index
 */
function nthChildOfType(element: Element, nodeName: string, index: number) {
 ...
 * @return {Node|null}
 */
export function nodeFromXPath(xpath: string, root: Element = document.body): Node | null {
  try {
