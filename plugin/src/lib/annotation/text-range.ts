 ...
 * @param {Node} node
 */
function previousSiblingsTextLength(node: Node) {
 ...
 */
function resolveOffsets(element: Element, ...offsets: number[]): { node: Text; offset: number }[] {
...
export const RESOLVE_FORWARDS = 1
export const RESOLVE_BACKWARDS = 2
...
export class TextPosition {
   ...
   * @throws {RangeError}
   */
  resolve(options: { direction?: typeof RESOLVE_FORWARDS | typeof RESOLVE_BACKWARDS } = {}): {
    node: Text
    offset: number
  } {
    try {
      ...
      if (this.offset === 0 && options.direction !== undefined) {
        ...
        const text = (forwards ? tw.nextNode() : tw.previousNode()) as Text | null
        if (!text) {
          throw err
        }
...
export class TextRange {
   ...
   * @param {number} end
   */
  static fromOffsets(root: Element, start: number, end: number) {
